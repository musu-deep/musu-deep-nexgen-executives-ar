...
const LOCAL_STATE_KEY = "__ARAAK_PASSWORD_DIRECTORY_V4__";
...
function seededState(reason = "initial_seed", previousVersion = null) {
  ...
  return {
    ...
    provisioning: {
      reason,
      previous_directory_version: previousVersion,
      completed_at: now(),
      user_count: users.length,
      temporary_credentials_reset: true,
    },
  };
}
  ...
  try {
    const response = await fetch(`${config.url}/json/2/${encodeURIComponent(model)}/${encodeURIComponent(method)}`, {
      ...
      headers: {
        ...
        "User-Agent": "ARAAK-CEO-AUTHORIZED-DIRECTORY/4.0",
      },
      body: JSON.stringify({
...
function directoryNeedsRebuild(state) {
  if (!state || state.directory_version !== DIRECTORY_VERSION || Number(state.version) !== STATE_VERSION) {
    return true;
  }

  const users = Array.isArray(state.users) ? state.users : [];
  if (users.length !== USER_ROSTER.length) return true;

  for (const profile of USER_ROSTER) {
    const user = users.find((item) => item.id === profile.id);
    if (!user) return true;
    if (String(user.email || "").trim().toLowerCase() !== profile.email.toLowerCase()) return true;
    if (user.must_change_password === true) {
      if (user.password_salt !== profile.temporary_salt || user.password_hash !== profile.temporary_hash) {
        return true;
      }
    }
  }
...
async function loadDirectory() {
  ...
  if (!config.apiKey) {
    const existing = globalThis[LOCAL_STATE_KEY];
    const mustRebuild = directoryNeedsRebuild(existing);
    if (mustRebuild) {
      globalThis[LOCAL_STATE_KEY] = seededState(
        existing ? "ephemeral_directory_upgrade" : "ephemeral_initial_seed",
        existing?.directory_version || null,
      );
    }
    return {
      state: globalThis[LOCAL_STATE_KEY],
      recordId: null,
      persistent: false,
      storage: "memory",
      migrationApplied: mustRebuild,
      migrationReason: globalThis[LOCAL_STATE_KEY]?.provisioning?.reason || null,
    };
  }
  ...
  if (!row) {
    if (config.readOnly) throw new Error("مخزن المستخدمين مضبوط على القراءة فقط ولا يحتوي دليلاً معتمداً.");
    const state = seededState("odoo_initial_seed", null);
    ...
    return {
      ...
      migrationReason: "odoo_initial_seed",
    };
  }

  const decoded = decodeState(row.description);
  const mustRebuild = directoryNeedsRebuild(decoded);
  let state = decoded;

  if (mustRebuild) {
    state = seededState("forced_v4_migration", decoded?.directory_version || null);
    if (!config.readOnly) {
      await writeOdooState(row.id, state);
    }
  }
  ...
  return {
    ...
    storage: "odoo",
    migrationApplied: mustRebuild,
    migrationReason: state?.provisioning?.reason || null,
  };
}
...
function findByLogin(state, email) {
  ...
  return (state.users || []).find((user) => String(user.email || "").trim().toLowerCase() === normalized);
}
...
export async function currentPasswordDirectoryUser(request) {
  ...
  if (!decoded || decoded.directory_version !== DIRECTORY_VERSION) return null;
...
export async function passwordDirectoryHealth() {
  const loaded = await loadDirectory();
  const users = Array.isArray(loaded.state?.users) ? loaded.state.users : [];
  return {
    status: directoryNeedsRebuild(loaded.state) ? "degraded" : "ready",
    storage: loaded.storage,
    persistent: loaded.persistent,
    read_only: odooConfig().readOnly,
    directory_version: loaded.state?.directory_version || null,
    expected_directory_version: DIRECTORY_VERSION,
    state_version: Number(loaded.state?.version || 0),
    user_count: users.length,
    active_user_count: users.filter((user) => user.active !== false).length,
    temporary_passwords_pending: users.filter((user) => user.must_change_password === true).length,
    migration_applied: loaded.migrationApplied,
    migration_reason: loaded.migrationReason,
    provisioned_at: loaded.state?.provisioning?.completed_at || null,
  };
}
...
export async function rebuildPasswordDirectory(actor) {
  requireAdmin(actor);
  const loaded = await loadDirectory();
  const previousVersion = loaded.state?.directory_version || null;
  loaded.state = seededState("manual_admin_rebuild", previousVersion);
...
export async function changeFirstLoginPassword(request, payload) {
  const authorization = request.headers.authorization || request.headers.Authorization || "";
  const decoded = parseToken(String(authorization).replace(/^Bearer\s+/i, ""));
  if (!decoded || decoded.directory_version !== DIRECTORY_VERSION) {
...
export const PASSWORD_DIRECTORY_VERSION = DIRECTORY_VERSION;
